/* gps.cpp copyright (C) Erwin Bejsta VK3ERW 2017
 * credit to Javi EA5HAV
 *
...
void parse_date(const char *token)
{
    ...
    yr[1] = token[5];
    yr[2] = '\0';
    int year = atoi(yr) + 2000;
    int month = ((token[2] - '0') * 10) + (token[3] - '0');
    int day = ((token[0] - '0') * 10) + (token[1] - '0');    
    sprintf(gps_date, "%02d%02d%04d",day, month, year);
    gps_date[9] = '\0';
    gps_date_received = true;
}
